...
def push_outputs_to_device(write_url, board, output_pin_types, last_pushed, timeout_s):
    ...
    DOUT semantics: any nonzero STATE["dout"][i] -> 1.0 (on), zero -> 0.0
    (off). Flash behavior is left to the Pi's own subtype-based logic — when
    we tried to alternate the state value here on every cycle to manufacture
